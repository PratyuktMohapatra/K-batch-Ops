...
REMOTE_SSH_HOST = "192.168.13.91"
REMOTE_SSH_USER = "pro"
REMOTE_SSH_PASS = "pro@2024"

HOST_IP = "192.168.95.29"
MICROK8S_CMD = ["microk8s", "kubectl"]
...
def delete_pod_and_service(name, namespace="default"):
    try:
        # Delete all pods with label app=name (deployment name)
        subprocess.run(MICROK8S_CMD + ["delete", "pods", "-l", f"app={name}", "-n", namespace], check=True)
        # Delete service by deployment name + "-svc"
...
def update_ips_in_database(client_id, frequency, vnc_node_port, web_node_port):
    container_ip = f"192.168.95.29:{vnc_node_port}"
    browser_container_ip = f"http://192.168.95.29:{web_node_port}"
